...
int* GetROI_TopBottom(vector<Vec4i> lines, double angle)
{
	...
	//统计每行白像素点数
	vector<double> point_nums(dst.rows);
	uchar* p;
	...
	//滤除干扰行
	double max_pnum = *max_element(point_nums.begin(), point_nums.end());
	vector<double> pnums_actual;
	for (int i = 0; i < point_nums.size(); i++) {
		if (point_nums[i] > 0.5 * max_pnum && point_nums[i] < 1.5 * max_pnum) {
			pnums_actual.push_back(point_nums[i]);
		}
	}

	//求条形码行序数的上下界限
	double range[2] = { 0.0, 1919.0 };
	vector<int> hist = GetHist(pnums_actual, 960, range);
	vector<double> vec_freq = GetHistFreq(hist, pnums_actual, range);
	double pnums_avr = 0;
	for (int i = 0; i < vec_freq.size(); i++) {
		pnums_avr += vec_freq[i] / vec_freq.size();
	}

	//const int countNum = 30; //每行像素点数大于countNum时开始计continNum
	//const int continNum = 10; //continNum表示连续几行像素点数大于countNum时视为边界
	const int countNum = 0.5 * pnums_avr; //每行像素点数大于countNum时开始计continNum
	const int continNum = 10; //continNum表示连续几行像素点数大于countNum时视为边界
